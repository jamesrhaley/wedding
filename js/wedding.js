...
JJWED.sidemenu = function () {
    "use strict";
    var wind = window,
        window_Doc = wind.document,
        navScroll,
        trueOrFalse = function (wind, window_Doc) {
            var value = -1,
                evaluation = ( value !== (" " + wind.className + " ").indexOf(" " + window_Doc + " ") );
            return evaluation;
        },
        trimName = function ( wind ) { // gets class names as menu opens
            var names = wind.trim ? wind.trim() : wind.replace(/^\s+|\s+$/g, "");
            return names;
        },
        getOpenClasses = function ( wind, window_Doc ) { // gets class names as menu closes
            var names = trueOrFalse(wind, window_Doc) || 
                    (wind.className = "" === wind.className ? window_Doc : wind.className + " " + window_Doc);
            return names;
        },
        getCloseClasses = function ( wind, window_Doc ) {
            var names = wind.className = trimName((" " + wind.className + " ").replace(" " + window_Doc + " ", " "));
            return names;
        },
        zeroOrOne = function ( wind, window_Doc ) {
            if (wind) {
                do {
    ...
    navScroll = (function () {
        ...
        navObject.closeNav = function () {
                ...
                getCloseClasses(element, addClassName);
                m = 0;
            };

        navObject.openNav = function () {
                m || ( getOpenClasses(element, addClassName), m = 1 );
            };

        navObject.toggleNav = function ( mouse_toggle ) {
                m && trueOrFalse( element, addClassName ) ? navObject.closeNav() : navObject.openNav();
        ...
        navObject.init = function () {
            if (!isOpen) {
                ...
                window_Doc.addEventListener("click", function ( mouse_toggle ) {
                    m && !zeroOrOne( mouse_toggle.target, "nav" ) && ( mouse_toggle.preventDefault(), navObject.closeNav() );
                }, 1);
                getOpenClasses( element, "js-ready" );
            }
        }
